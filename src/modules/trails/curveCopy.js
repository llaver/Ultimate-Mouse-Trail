...
class CurveCopy {
  ...
  makeMore() {
    for (let i = 2; i < this.positions.length - 2; i + 2) {
      this.ctx.beginPath();
      //this.ctx.moveTo(this.positions[i - 2].x, this.positions[i - 2].y);
      let curveSegment = curve(this.ctx, [
      ...
      console.log(curveSegment);
      // if (curve) {
      //   this.ctx.lineTo(curve[0], curve[1]);
      //   this.ctx.lineTo(curve[2], curve[3]);
      //   this.ctx.lineTo(curve[4], curve[5]);
      // }
